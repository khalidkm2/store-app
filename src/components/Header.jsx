...
const Header = () => {
  ...
  const searchedProducts = products.filter((product) =>{
    return product.title.toLowerCase().includes(searchText.current.value.toLowerCase()) || product.category.toLowerCase().includes(searchText.current.value.toLowerCase())
  } )
  ...
  return (
    ...
    <div className=' w-6/12 flex justify-between'>
        <input onChange={handleAll} ref={searchText} type='text' className=' border border-gray-400 focus:outline-gray-600 mr-3 active:border-blue-800 text-gray-700 w-full p-1 rounded-sm' />
